...
然后在Virtual state的物质回Relax（1）回到原始态，放出与入射光相同的光子，被称为Rayleigh Scattering。（2）回到一个高能态，放出能量较低的光子，被称为Stokes Scattering。（3）被激发到一个高能虚拟态，然后回到原始态，被称为Anti-Stokes Scattering。  
